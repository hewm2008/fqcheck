...
int RunSingleFq( string InFq, string OutStat,  Para_A24 * para  )
{
	...
	static	LLongA base[5];//记录每种碱基的总数
	static	LLongA qual[MAX_Q];//每个质量值的总频率
	static	LLongA cycle_qual[MAX_L][MAX_Q];//每个cycle的质量分布
	static	LLongA cycle_base[MAX_L][5];//每个cycle的碱基分布
...
int PlotfqCheck ( vector  <string> infile , string outfile,string ver )
{
	...
	else
	{
		...
		popen(path.c_str(),"r") ;
		sleep(3);
		//popen(path.c_str(),"r"); // waste time for pdf done		
